	...
	for i := 0; i < 4; i++ {
		...
		case seguindoCarregados := <- canalSeguindo:
			if seguindoCarregados == nil {
				return Usuario{}, errors.New("Erro ao buscar seguido!")
			}
		...
		case publicacoesCarregado := <- canalPublicacoes:
			if publicacoesCarregado == nil {
				return Usuario{}, errors.New("Erro ")
			}	
...
// BuscarSeguindo = Chama a API para buscar os usuários seguidos por outro usuário
