...
func Login(w http.ResponseWriter, r *http.Request) {
    ...
    if erro := json.NewDecoder(r.Body).Decode(&credenciais); erro != nil {
    ...
    usuario, erro := repositorio.BuscarPorEmail(credenciais.Email)
    if erro != nil {
        respostas.Erro(w, http.StatusInternalServerError, erro)
        return
    }

    if erro = bcrypt.CompareHashAndPassword([]byte(usuario.Senha), []byte(credenciais.Senha)); erro != nil {
        respostas.Erro(w, http.StatusUnauthorized, errors.New("Senha ou email inválidos"))
        return
    }
...
// SeguirUsuario = Permite que um usuário siga os demais
