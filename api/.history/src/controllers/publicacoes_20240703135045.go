...
// CriarPublicacao adiciona uma nova publicação no banco de dados
	...
	if erro = json.Unmarshal(corpoRequisicao, &publicacao); erro != nil { // Adicionado json.Unmarshal
		respostas.Erro(w, http.StatusBadRequest, erro) // Corrigido de "htttp" para "http"
		return
	}
