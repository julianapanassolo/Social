...
// CriarPublicacao adiciona uma nova publicação no banco de dados
	...
	func BuscarPublicacao(w http.ResponseWriter, r *http.Request {
		parametros := mux.Vars()
		publicacaoID, erro := strconv.ParUint(parametros["publicacaoId"], 10, 64)
		if erro != nil {
			respostas.Erro(w, http.StatusBadRequest, erro)
			return
		}
		...
		repositorio := repositorio.NovoRepositorioDePublicacoes(db)
		publicacao, erro := repositorio.BuscarID(publicacaoID)
		if erro != nil {
			respostas.Erro(w, http.Status, erro)
			return
